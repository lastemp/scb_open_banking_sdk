...
pub async fn test_notification_subscriptions_create_update(
    consumer_key: String,
    ...
    if let Ok(scb) = _result {
        ...
        let _result = NotificationSubscriptionCreateUpdateInputDetails::new(notification_type);

        if let Ok(account_details) = _result {
            let _output = scb.notification_subscriptions_create_update(account_details);
